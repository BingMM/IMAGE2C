...
'''
for orbit in tqdm(o, total=len(o), desc='Loop over orbits'):

    # Load nc orbit files
    wic_nc = netcdf_file(p_wic_nc + 'wic_or' + str(orbit).zfill(4) + '.nc', 'r')
    s12_nc = netcdf_file(p_s12_nc + 's12_or' + str(orbit).zfill(4) + '.nc', 'r')
    s13_nc = netcdf_file(p_s13_nc + 's13_or' + str(orbit).zfill(4) + '.nc', 'r')
    ...
    # Get common times and their indices in their respective files
    t, indices = find_common_times_with_indices(t_wic, t_s12, t_s13)
    
    # Remove completely empty frames
    empty = np.all(np.isnan(wic_nc.variables['mlat'][indices[:, 0]]), axis=(1,2))
    empty = empty | np.all(np.isnan(s12_nc.variables['mlat'][indices[:, 1]]), axis=(1,2))
    empty = empty | np.all(np.isnan(s13_nc.variables['mlat'][indices[:, 2]]), axis=(1,2))
    ...
    ##### Implemented after native mlat mlon were incompatible with apexpy (31/03/2025)
    ##### Modified to avoid NaN input warning (27/05/2025)
    for i, ti in enumerate(t):
        apex = apexpy.Apex(ti)
        wic_pI.mlat[i], wic_pI.mlon[i] = safe_apex_convert(apex, wic_pI.glat[i], wic_pI.glon[i])
        s12_pI.mlat[i], s12_pI.mlon[i] = safe_apex_convert(apex, s12_pI.glat[i], s12_pI.glon[i])
    ...
    # Remove instances of empty images
    wic_p = wic_pI.percent_full(grid_w)
    s12_p = s12_pI.percent_full(grid_s)
    s13_p = s13_pI.percent_full(grid_s)
    
    f_thres = .1 # fullness threshold.
    f = (wic_p >= f_thres) & (s12_p >= f_thres) & (s13_p >= f_thres)
    ...
    # Make conductance images
    cI = ConductanceImage(wic_bI, s12_bI, s13_bI, time=t)
    
    # Save to netcdf
    cI.to_nc('/disk/IMAGE_FUV/fuv/conductance/or_' + str(orbit).zfill(4) + '.nc')
'''
...
ax = axs[0,0]
ax.set_title('R')
im = ax.imshow(cI.R[50])
fig.colorbar(im, ax=ax, orientation='vertical')

ax = axs[0,1]
ax.set_title('dR')
im = ax.imshow(cI.dR[50])
fig.colorbar(im, ax=ax, orientation='vertical')

ax = axs[1,0]
ax.set_title('E0')
im = ax.imshow(cI.E0[50])
fig.colorbar(im, ax=ax, orientation='vertical')

ax = axs[1,1]
ax.set_title('dE0')
im = ax.imshow(cI.dE0[50])
fig.colorbar(im, ax=ax, orientation='vertical')

ax = axs[2,0]
ax.set_title('Fe')
im = ax.imshow(cI.Fe[50])
fig.colorbar(im, ax=ax, orientation='vertical')

ax = axs[2,1]
ax.set_title('dFe')
im = ax.imshow(cI.dFe[50])
fig.colorbar(im, ax=ax, orientation='vertical')

plt.savefig('/Home/siv32/mih008/Downloads/stats1.png', bbox_inches='tight')
plt.close('all')
...
ax = axs[0,0]
ax.set_title('H')
im = ax.imshow(cI.H[50])
fig.colorbar(im, ax=ax, orientation='vertical')

ax = axs[0,1]
ax.set_title('dH')
im = ax.imshow(cI.dH[50])
fig.colorbar(im, ax=ax, orientation='vertical')

ax = axs[1,0]
ax.set_title('P')
im = ax.imshow(cI.P[50])
fig.colorbar(im, ax=ax, orientation='vertical')

ax = axs[1,1]
ax.set_title('dP')
im = ax.imshow(cI.dP[50])
fig.colorbar(im, ax=ax, orientation='vertical')

plt.savefig('/Home/siv32/mih008/Downloads/stats2.png', bbox_inches='tight')
plt.close('all')
plt.ion()
'''
...
#loaded_cI = conductanceImage(filename='/home/bing/Dropbox/work/temp_storage/cI_test.nc')
